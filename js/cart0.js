...
function changeProductQty(delta) {
    ...
    let value =
        (parseInt(input.value) || min) + delta;
...
function updateProductSubtotal() {
    ...
    const subtotalEl =
        document.getElementById('product-subtotal');
...
function handleOrderSubmit(event) {
    ...
    if (
        paymentValue === 'lumicash' ||
        paymentValue === 'ecocash'
    ) {
        ...
        if (!transactionRef) {
            ...
            alert(
                'Veuillez indiquer le code de transaction reçu par SMS après votre paiement ' +
                paymentLabel +
                '.'
            );
